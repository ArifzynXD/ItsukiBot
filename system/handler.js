...
export async function handler(conn, m, chatUpdate) {
    ...
    try {
        ...
        if (!m.isBaileys && !m.fromMe) console.log("\x1b[1;31m~\x1b[1;37m>", "[\x1b[1;32m CMD \x1b[1;37m]", chalk.yellow(m.type), "from", chalk.green(m.pushName), "in", chalk.cyan(m.isGroup ? m.metadata.subject : "private chat" ), "args :", chalk.green(m.body.length))
    }
}
...
export async function deleteUpdate({ fromMe, id, participants }) {
    try {
        ...
        conn.sendMessage(msg.key.remoteJid, { text: `[❗] Terdeteksi @${participants[0].split("@")[0]} telah menghapus pesan.\n\nUntuk mematikan fitur ini, ketik *.off antidelete*\nUntuk menghapus pesan yang dikirim oleh BOT, balas pesan dengan perintah *.delete*`, mentions: [participants[0]] }, { quoted: msg })
